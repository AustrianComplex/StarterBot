...
var defaultserversetup = function(server)
{
    ...
    var channels = server.channels.cache.array();
    ...
    var roles = server.roles.cache.array();
    for(var i = 0; i < roles.length; i++)
    {
        if(roles[i].members.array().length == 0)
        {
            try{
                deletesync(roles[i]);
            }
    ...
    server.channels.create("INFORMATION", {type: "category"}).then(channel => {
        ...
        server.channels.create("announcements", {type: "text"}).then(chann => {
            chann.setParent(informationid);
        }).catch(error => {});
        server.channels.create("new-players", {type: "text"}).then(chann => {
            chann.setParent(informationid);
            admin.sendmessage(defaultdescription, chann);
        }).catch(error => {});
        server.channels.create("rules", {type: "text"}).then(chann => {
            chann.setParent(informationid);
            admin.sendmessage(defaultrules, chann);
        }).catch(error => {});
        server.channels.create("questions", {type: "text"}).then(chann => {
            chann.setParent(informationid);
        }).catch(error => {});
    }).catch(error => {});
    ...
    server.channels.create("GAME", {type: "category"}).then(channel => {
        gameid = channel.id;
        channel.updateOverwrite(channel.guild.roles.everyone, { SEND_MESSAGES: false }).catch(console.log);
        server.channels.create("day", {type: "text"}).then(chann => {
            chann.setParent(gameid);
        }).catch(error => {});
        server.channels.create("night", {type: "text"}).then(chann => {
            chann.setParent(gameid);
        }).catch(error => {});
        server.channels.create("voting", {type: "text"}).then(chann => {
            chann.setParent(gameid);
        }).catch(error => {});
        server.channels.create("player-shoutouts", {type: "text"}).then(chann => {
            chann.setParent(gameid);
            chann.send(defaultshoutoutdescription);
        }).catch(error => {});
        server.channels.create("last-wills", {type: "text"}).then(chann => {
            chann.setParent(gameid);
        }).catch(error => {});
        server.channels.create("graveyard", {type: "text"}).then(chann => {
            chann.setParent(gameid);
        }).catch(error => {});
    }).catch(error => {});
    ...
    server.channels.create("OFF TOPIC", {type: "category"}).then(channel => {
        ...
        server.channels.create("memes", {type: "text"}).then(chann => {
    ...
    var hostid;
    server.channels.create("HOST CHANNELS", {type: "category"}).then(channel => {
        channel.updateOverwrite(channel.guild.roles.everyone, { VIEW_CHANNEL: false }).catch(console.log);
        hostid = channel.id;
        server.channels.create("host-updates", {type: "text"}).then(chann => {
