...
var createGame = function(chosengamename, ownerid)
{
    ...
    fs.appendFileSync(`./gamemap.txt`, `${id}  ${gamename}\n`);
...
var deleteGame = function(gameid)
{
    try
    {
        ...
        fs.writeFileSync(`./gamemap.txt`, fs.readFileSync(`./gamemap.txt`).toString().replace(`${gameid}  ${gamename}\n`, ""));
        fs.rmdirSync(`${dir}/${gameid}`, { recursive: true });
    }
    catch(err)
    {
        console.error(err);
...
{
    var rawgamemapdata = fs.readFileSync(`./gamemap.txt`).toString().split("\n");
...
{
    var rawservermapdata = fs.readFileSync(`./servermap.txt`).toString().split("\n");
...
{
    var games = [];
    var gameids = fs.readFileSync(`./gamemap.txt`).toString().split("\n");
...
var addServer = function(gameid, serverid)
{
    ...
    fs.appendFileSync(`./servermap.txt`, `${serverid}  ${gameid}\n`);
}
...
var removeServer = function(gameid, serverid)
{
    ...
    fs.writeFileSync(`./servermap.txt`, fs.readFileSync(`./servermap.txt`).toString().replace(`${serverid}  ${gameid}\n`, ""));
}
